...
from Models import ResNet18, Mnist_CNN, QuantumByzantineDetector
...
def Feature_extraction_model(Central_par, dev):
    ...
    if args['data_name'] == 'mnist':
        FC['conv1.weight'], Std['conv1.weight'], Dis['conv1.weight'] = train_vqc(
            kernel1_train, 10 * 1 * 5 * 5, dev
        )
        FC['fc.weight'], Std['fc.weight'], Dis['fc.weight'] = train_vqc(
            weight3_train, 10 * 320, dev
        )

    elif args['data_name'] == 'cifar_10':
        FC['conv1.weight'], Std['conv1.weight'], Dis['conv1.weight'] = train_vqc(
            kernel1_train, 64 * 3 * 3 * 3, dev
        )
        FC['fc.weight'], Std['fc.weight'], Dis['fc.weight'] = train_vqc(
            weight3_train, 10 * 512, dev
        )
...
def train_vqc(weight_train, dimen, dev):
    """Train a QuantumByzantineDetector on central server weight tensors.

    This function replaces ``train_Linear`` from FLAD.  The interface and return
    values are identical so that the rest of the pipeline is unchanged.

    Parameters
    ----------
    weight_train : torch.Tensor  Raw weight tensors from central training.
    dimen        : int           Flattened feature dimension (used for model init).
    dev          : torch.device  Main training device (CUDA or CPU).

    Returns
    -------
    (model, std, dis) — trained QuantumByzantineDetector, feature mean, feature range.
    """
    # Quantum circuits run on CPU; flatten and move to CPU
    ...
    test_model = QuantumByzantineDetector(dimen)
    # Note: QuantumByzantineDetector always operates on CPU (quantum simulator)
    optimizer = optim.Adam(test_model.parameters(), lr=0.001)
    criterion_test = torch.nn.MSELoss(reduction='sum')
    label = torch.tensor([[1.0]])

    # Training loop (same structure as FLAD's train_Linear)
...
def neural_network_feature_extraction(Upload_Parameters, FC, Std, Dis, dev):
    ...
    # QuantumByzantineDetector handles CPU transfer internally
    feature = np.zeros([args['num_of_clients'], 2])
    feature[:, 0] = (
        FC['conv1.weight'](kernel1.view(args['num_of_clients'], -1))
        .cpu()
        .detach()
        .numpy()
        .reshape(args['num_of_clients'],)
    )
    feature[:, 1] = (
        FC['fc.weight'](weight3.view(args['num_of_clients'], -1))
        .cpu()
        .detach()
        .numpy()
        .reshape(args['num_of_clients'],)
        ...
        if label != -1:
            for client in range(args['num_of_clients']):
                if label_list[client] == label:
                    temp = np.concatenate(
                        (temp, feature[client, :].reshape(1, 2)), axis=0
                    )
            ...
            length = abs(
                np.sqrt(np.sum(label_mean[label] * label_mean[label].T)) / honest_L2 - 1.0
            )
...
def FedAvg(Upload_Parameters, malicious):
    count = 0
    # It is important to ensure that the malicious client indices are sorted in ascending order
    for j in malicious:
    ...
    for key in sum_parameters:
        sum_parameters[key] = sum_parameters[key] / (
            args['num_of_clients'] - len(malicious)
        )
    return sum_parameters


parser = argparse.ArgumentParser(description='QBAD-FL')
...
parser.add_argument('-iid', '--IID', type=bool, default=True,
                    help='the way to allocate data to clients')
parser.add_argument('-cen', '--central_data_size', type=int, default=300,
                    help='central data size in server')
parser.add_argument('-pro', '--central_data_pro', type=float, default=0.1,
                    help='central data pro in server')
parser.add_argument('-alpha', '--alpha', type=float, default=0.5,
                    help='importance weights for direction and length')
...
if __name__ == "__main__":
    ...
    honest_clients = [
        'client{}'.format(i)
        for i in range(args['num_of_clients'] - args['byzantine_size'])
    ]
    byzantine_clients = [
        'client{}'.format(i)
        for i in range(
            args['num_of_clients'] - args['byzantine_size'], args['num_of_clients']
        )
    ]
    ...
    for i in range(args['num_comm']):
        ...
        for client in honest_clients:
            local_parameters = myClients.clients_set[client].localTrain(
                args['epoch'], args['batchsize'], net, loss_func, opti, global_parameters
            )
            ...
            if args['pattern'] <= 2 or args['pattern'] >= 5:
                if honest_all_weight is None:
                    ...
                    for key in local_parameters:
                        honest_all_weight[key] = torch.cat(
                            [
                                honest_all_weight[key],
                                local_parameters[key].unsqueeze(0),
                            ],
                            dim=0,
                        )
        ...
        for client in byzantine_clients:
            ...
            if args['pattern'] == 2:
                for key in honest_all_weight:
                    local_parameters[key] = Attack.ZeroGradient_attack(
                        honest_all_weight[key], args['byzantine_size']
                    )
                ...
                for key in honest_all_weight:
                    local_parameters[key] = Attack.Sign_flipping_attack(
                        honest_all_weight[key]
                    )

            elif args['pattern'] == 3:
                Poisoning_client = Attack.backdoor_poisoning_data(
                    myClients.clients_set[client], args['data_name']
                )
                local_parameters = Poisoning_client.localTrain(
                    args['epoch'], args['batchsize'], net, loss_func, opti, global_parameters
                )
            elif args['pattern'] == 4:
                Poisoning_client = Attack.model_replacement_attack_data(
                    myClients.clients_set[client], args['data_name']
                )
                local_parameters = Poisoning_client.localTrain(
                    args['epoch'], args['batchsize'], net, loss_func, opti, global_parameters
    ...
    torch.save(
        ...
        os.path.join(
            args['save_path'],
            '{}_num_comm{}_E{}_B{}_lr{}_num_clients{}_cen{}_attack{}'.format(
                args['data_name'],
                args['num_comm'],
                args['epoch'],
                args['batchsize'],
